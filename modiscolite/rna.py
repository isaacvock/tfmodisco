...
def infer_padding_mask(sequence, sequence_mask=None, progress=None):
...
def infer_region_mask(sequence, padding_mask, region="all", missing_cds="skip",
	...
	with progress.task(
		"Inferring {} annotations".format(region),
		total=sequence.shape[0],
		unit="RNA",
	) as task:
...
def candidate_window_mask(position_mask, window_size, flank, progress=None):
	...
	with progress.task(
		"Identifying valid seqlet windows",
		total=n_windows,
		unit="window",
	) as task:
		for window_start in range(n_windows):
			...
			if seqlet_start >= 0 and seqlet_end <= length:
				mask[:, window_start] = (
